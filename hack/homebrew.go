...
func main() {
    ...
    "bin": "eks-node-viewer",
    "bottle": {
        "root_url": "https://github.com/brunokktro/eks-node-viewer-for-windows/releases/download/v{{.Version}}/eks-node-viewer",
        "sha256": {
            "sierra": "{{.DarwinAll}}",
            "linux": "{{.LinuxX64}}",
            "linux_arm": "{{.LinuxArm}}"
...
`)
	...
	req, err := http.Get(fmt.Sprintf(`https://github.com/brunokktro/eks-node-viewer-for-windows/releases/download/v%s/eks-node-viewer_%s_sha256_checksums.txt`, data.Version, data.Version))
	if err != nil {
